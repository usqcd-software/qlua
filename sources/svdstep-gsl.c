...
static void
svd2 (gsl_vector * d, gsl_vector * f)
{
...
static void
chase_out_intermediate_zero (gsl_vector * d, gsl_vector * f, size_t k0)
{
  const size_t n = d->size;
  double c, s;
  double x, y;
...
static void
chase_out_trailing_zero (gsl_vector * d, gsl_vector * f)
{
  const size_t n = d->size;
  double c, s;
  double x, y;
...
static void
qrstep (gsl_vector * d, gsl_vector * f)
{
  const size_t n = d->size;
  double y, z;
  ...
  for (i = 0; i < n - 1; i++)
    {
      ...
      if (d_i == 0.0)
        {
          chase_out_intermediate_zero (d, f, i);
          return;
        }
    }
